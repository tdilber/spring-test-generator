...
@Slf4j
public class GetEntityDTOMethodCreator extends BaseMethodCreator {
    public GetEntityDTOMethodCreator(IntegrationTestGenerator integrationTestGenerator) {
    ...
    @Override
    public String name() {
        return "Get Entity(DTO)";
    }

    @Override
    public String templatePath() {
        return "/integration_test/methods/GetDTOITMethod.tempjava";
    }

    @Override
    public boolean isAvailable(Class<?> resourceClass, RequestMappingInfo requestMappingInfo, HandlerMethod handlerMethod) {
        if (handlerMethod.getMethod().getName().startsWith("get") && handlerMethod.getMethod().getParameterTypes().length == 1 && Long.class.isAssignableFrom(handlerMethod.getMethod().getParameterTypes()[0])) {
...
//                    expectsSB.append("        ").append(templateValueMap.get(eIntegrationTestMethodVariable.EntityVariable)).append(".")
//                            .append(propertyDescriptor.getName()).append("(").append(valueStr).append(");\n");
